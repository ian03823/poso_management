...
<head>
  ...
  <title>Reset Enforcer Password</title>
...
<body>
  ...
  <main class="pw-page d-flex align-items-center justify-content-center">
    <div class="pw-card shadow-lg">
      ...
      @endif
      @if(session('error'))
        <div class="alert alert-warning text-center py-2 mb-3">{{ session('error') }}</div>
      @endif

      <h1 class="h5 text-center fw-bold mb-3 text-success">Reset Password</h1>
      <p class="text-center text-muted small mb-4">
        Set a new password to continue using the Enforcer app.
      </p>

      <form method="POST" action="{{ route('enforcer.password.update') }}" novalidate>
        @csrf
  ...
  <!-- SweetAlert (only used when forced) -->
  <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>

  <script>
    // Show alert only when forced by session
    @if(session('error') || session('force_pwd_change'))
    ...
    function paint() {
      ...
      if (pc.value.length) {
        ...
        matchHint.className = 'form-text small';
      }
    }
  ...
  <!-- Bootstrap JS (optional; not required for this page’s logic) -->
